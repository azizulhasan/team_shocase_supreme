<?php
if(!function_exists('wpm_member_actions')){
	function wpm_member_actions($id){
		echo "<span class='wpm_6310_member_edit' data-id='{$id}'>Edit</span>
		<span class='wpm_6310_member_delete' data-id='{$id}'>Delete</span>";
	}
}
if (!function_exists('wpm_6310_category_menu')) {
	function wpm_6310_category_menu($categoryData, $styleId, $cls = '')
	{
		$carousel = ($cls)?'-carousel':'';
		echo "<div id='wpm-6310-category{$carousel}-{$styleId}' class={$cls}>";
		$index = 1;
		foreach($categoryData as $value){
			$active = ($index)?" wpm_6310_category_list_active":'';
			echo "<div class='wpm_6310_category_list wpm_6310_category_list_{$styleId} {$active}' wpm-data-filter='{$value['c_name']}'>{$value['name']}</div>";
			$index = 0;
		}
		echo "</div>";
	}
...
if (!function_exists('wpm_6310_team_member_info')) {
	function wpm_6310_readmore_activation($template_id, $profile_details_type, $text)
	{
		if($profile_details_type > 0){
			echo "<div class='wpm_6310_team_style_readmore wpm_6310_team_style_{$template_id}_readmore'>
                    <div class='wpm_6310_team_style_{$template_id}_readmore_text'>{$text}</div>
                    </div>";
		}
	}
}

if(!function_exists('wpm_6310_team_member_delete')){
	function wpm_6310_team_member_delete(){
		global $wpdb;
		$id = (int) sanitize_text_field($_GET['ids']);
		$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}wpm_6310_member WHERE id = %d", $id));
		wp_die();
	}
}
...
if (!function_exists('wpm_check_field_exists')) {
	function wpm_check_field_exists(){
		global $wpdb;
		$member_table = $wpdb->prefix . 'wpm_6310_member';
		$category_table = $wpdb->prefix . 'wpm_6310_category';

		$result = $wpdb->query("SHOW COLUMNS FROM {$member_table} LIKE 'category'");
		if(!($wpdb->num_rows)){
			$wpdb->query("alter table {$member_table} add (category text DEFAULT NULL, custom_fields text DEFAULT NULL)");
		}
...
if (!function_exists('wpm_team_showcase_supreme_install')) {
	function wpm_team_showcase_supreme_install()
	{
		...
		if (!($memberData)) {
			...
			$results = $wpdb->get_results('SELECT * FROM ' . $category_table . ' ORDER BY serial ASC', ARRAY_A);
			if($results){
				foreach ($results as $value) {
					...
					else if($value['serial'] == 2){
						$category3 .= $value['c_name'];
						$category4 .= $value['c_name'];					
					}
					else if($value['serial'] == 3){
						$category1 .= $value['c_name'];
						$category2 .= $value['c_name'];
					}	
					else if($value['serial'] == 4){
						$category2 .= $value['c_name'];
						$category3 .= $value['c_name'];
					}	
					else{
			...
			$sql = "INSERT INTO {$member_table} (name, designation, profile_details_type, profile_url, open_new_tab, profile_details, effect, image, iconids, iconurl, category) VALUES
			('Adam Smith', 'CEO', '2', '', '0', '{$details}', 'top', 'https://www.wpmart.org/wp-content/uploads/2020/04/1.jpg', '1,37,24', 'https://www.linkedin.com||||admin@gmail.com||||+123456789', '{$category1}'),
			('George Michel', 'Sales Agent', '2', '', '0', '{$details}', 'top', 'https://www.wpmart.org/wp-content/uploads/2020/04/2.jpg', '2,37,38', 'https://www.facebook.com||||admin@gmail.com||||+123456789', '{$category2}'),
			('Jeson Smith', 'Web Developer', '2', '', '0', '{$details}', 'top', 'https://www.wpmart.org/wp-content/uploads/2020/04/3.jpg', '2,1,24', 'https://www.facebook.com||||https://www.linkedin.com||||+123456789', '{$category3}'),
			('Jones Lee', 'Web Designer', '2', '', '0', '{$details}', 'top', 'https://www.wpmart.org/wp-content/uploads/2020/04/4.jpg', '27,1,25', 'https://www.facebook.com||||admin@gmail.com||||+123456789', '{$category4}')";
...
if (!function_exists('wpm_check_license')) {
	function wpm_check_license($key)
	{
		global $wpdb;
