...
function tss_shortcode_extract_function($ids) {
    ...
    if (file_exists(team_showcase_supreme_plugin_url . "public/output-{$styledata['style_name']}.php")) {
        ...
        if (strpos($styledata['style_name'], 'style') !== false) {
            wp_enqueue_style("tss-google-font-{$ids}", "https://fonts.googleapis.com/css?family={$allStyle['17']}|{$allStyle['23']}|{$allSlider['33']}");
        } else if (strpos($styledata['style_name'], 'square') !== false) {
            wp_enqueue_style("tss-google-font-{$ids}", "https://fonts.googleapis.com/css?family={$allStyle['12']}|{$allStyle['19']}|{$allSlider['33']}");
        }
